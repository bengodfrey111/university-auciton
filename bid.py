...
def newBid(itemID, username, maxPrice): #this will place a new bid the user makes
    ...
    try:
        if bidExist: #checks if user already made a bid and wants to make another bid, if bid already made just edit it, else make new record of the bid
            connection.execute("UPDATE Bids SET maxPrice=:maxPrice WHERE itemID=:itemID AND username=:username", {"maxPrice": maxPrice, "itemID": itemID, "username": username})
        else:
            connection.execute("INSERT INTO Bids (maxPrice, itemID, username) VALUES (:maxPrice, :itemID, :username)", {"maxPrice": maxPrice, "itemID": itemID, "username": username})
        connection.commit()
        connection.close()
        return True
...
def maxUserBid(itemID, username): #this retreives the max bid of a specific user
    ...
    if bidExist:
        ...
        for row in cursor:
            return {"maxBid": str(row[0]), "username": str(row[1]), "itemID": str(row[2])}
    else:
        return "0.00"
...
def finalPrice(itemID):
    ...
    if float(max["maxBid"]) - 0.1 > float(secondMax["maxBid"]):
        return {"currentPrice": str(float(secondMax["maxBid"]) + 0.1), "username": max["username"], "itemID": max["itemID"]} #this gives a 10p increase of second highest bid
    else:
        return {"currentPrice": max["maxBid"], "username": max["username"], "itemID": max["itemID"]} #this just gives the max bid
